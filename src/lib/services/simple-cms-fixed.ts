...
/**
 * FIXED VERSION - Compatible with Hybrid Translation Schema
 *
 * This version works with the new schema where:
 * - PageTranslation only has SEO fields (no content field)
 * - Content is stored in PageContent as key-value pairs
 */
...
export interface CreatePageData {
  slug: string;
  title: string;
  content: string; // Will be stored in PageContent, not PageTranslation
  excerpt?: string;
  featuredImage?: string;
  languageId: string;
}
...
export interface UpdatePageData {
  title?: string;
  content?: string; // Will be stored in PageContent
  excerpt?: string;
  featuredImage?: string;
}
...
export class SimpleCMS {
  ...
  static async getPageBySlug(
    ...
    return {
      ...
      createdAt: page.createdAt,
      updatedAt: page.updatedAt,
      pageContent: this.parseDynamicContent(page.translations[0].pageContent),
    };
  }

  /**
   * Create new page (FIXED for hybrid schema)
   */
  static async createPage(data: CreatePageData): Promise<SimplePage> {
    // Create page with translation (SEO fields only)
      ...
      data: {
        ...
        translations: {
          create: {
            ...
            seoTitle: data.title, // Use title as seoTitle by default
    ...
    // Store content in PageContent (if provided)
    if (data.content && page.translations[0]) {
  ...
  /**
   * Update page (FIXED for hybrid schema)
   */
  static async updatePage(
    ...
    // Update translation (SEO fields only)
    if (data.title || data.excerpt) {
      await prisma.pageTranslation.update({
        where: { id: page.translations[0].id },
        data: {
          title: data.title,
          excerpt: data.excerpt,
        },
    ...
    // Update content in PageContent (if provided)
    if (data.content) {
      await prisma.pageContent.upsert({
        where: {
          pageTranslationId_key: {
    ...
    // Fetch updated page
    return this.getPageById(pageId, languageId);
  }
