...
const defaultImages: CommunityStoriesSectionProps["images"] = [
  {
    id: "1",
...
> = ({ leftText: propLeft, rightText: propRight, images: propImages }) => {
  const { getValue, getContentJSON } = usePageContentHelpers();

  const leftText = getValue("communityStories.leftText", propLeft, defaultLeft);
  const rightText = getValue(
    "communityStories.rightText",
    propRight,
    defaultRight
  );
  const images = getContentJSON(
    "communityStories.images",
    propImages ?? defaultImages
  );

  return (
    <section className="py-12 sm:py-20 bg-[#F7FAFC]">
      <div className="container mx-auto px-4">
        {/* Image Collage — driven by STORIES page CMS */}
        {images.length > 0 && (
          <div className="mb-10 sm:mb-12">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-0 overflow-hidden rounded-lg shadow-lg">
              {images.map((image) => (
                <div key={image.id} className="relative h-64 sm:h-80 md:h-96">
                  <Image
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 lg:gap-12">
          <H2
            style="h5regular"
            className="text-[#060726CC] leading-[1.4] col-span-1 sm:col-span-4 text-xl sm:text-2xl"
          >
            {leftText}
          </H2>
          ...
          <P
            style="p1reg"
            className="text-[#060726CC] col-span-1 sm:col-span-8"
          >
